...
class RequestController {
  async get(req, res) {
    const data = await db.getData('article_view');
    const results = [];
  ...
  async write(req, res) {
    ...
    if (obj.sectionId == null) {
      const query = db.generateInsertQuery('section', { type: 3 });
      db.connection.query(query, async function (err, result) {
        ...
        const insertedRow = (await db.getConditionData('article_view', { sectionId: result.insertId }))[0];
      ...
      const article = (await db.getConditionData('article_view', { sectionId: obj.sectionId }))[0];
  ...
  async addPicture(req, res, next) {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      db.connection.query(query, async function (err, results) {
        ...
        fs.copyFile(oldpath, newpath, function (err) { if (err) console.error(err); });
        res.status(200).send();
      });
